...
- MCP Forgebird 脚手架升级：在 `core/schema` 增补解析器、转换器与处理器三件套，支持外部/相对 `$ref`、`nullable`/`not`/`discriminator` 以及跨文件 `$defs` 的解析与裁剪，并在组件工厂构建前自动完成校验与优化；同时暴露操作中间件、组件钩子与自定义路由映射器，支撑多协议统一执行链。参见 `../mcp-forgebird/core/schema/resolver.go:1`、`../mcp-forgebird/core/schema/converter.go:1`、`../mcp-forgebird/core/schema/processor.go:10`、`../mcp-forgebird/core/factory/factory.go:72`、`../mcp-forgebird/core/forgebird.go:12`、`../mcp-forgebird/core/interfaces/plugin.go:128`。配套回归测试位于 `../mcp-forgebird/core/schema/processor_test.go:1`、`../mcp-forgebird/core/factory/factory_test.go:1`、`../mcp-forgebird/core/forgebird_test.go:234`，`go test ./...` 已通过。
- 工具注解策略可插拔：`DefaultComponentFactory` 新增 `WithToolAnnotationStrategy`，`Forgebird` 公开 `UseToolAnnotationStrategy`，默认提供基于 HTTP verbs 的实现，但允许插件替换为任意协议自定义注解。参见 `../mcp-forgebird/core/interfaces/factory.go:12`、`../mcp-forgebird/core/factory/factory.go:20`、`../mcp-forgebird/core/factory/annotation_strategy.go:1`、`../mcp-forgebird/core/forgebird.go:20`，测试 `../mcp-forgebird/core/factory/factory_test.go:73`、`../mcp-forgebird/core/forgebird_test.go:333` 覆盖。
- 组件描述/URI/元数据策略可插拔：`DefaultComponentFactory` 支持 `WithComponentDescriptorStrategy`，`Forgebird` 暴露 `UseComponentDescriptorStrategy`，默认改为协议无关的 `NewGenericComponentDescriptorStrategy`，HTTP 语义通过可选策略提供。参见 `../mcp-forgebird/core/interfaces/factory.go:23`、`../mcp-forgebird/core/factory/component_descriptor_strategy.go:1`、`../mcp-forgebird/core/factory/factory.go:19`、`../mcp-forgebird/core/forgebird.go:22`，测试 `../mcp-forgebird/core/factory/factory_test.go:115`、`../mcp-forgebird/core/forgebird_test.go:368`。
- 路由映射默认改为协议无关：`GetDefaultMappingRules` 现返回空集合，新增 `GetHTTPMappingRules` 与 `mapper.NewHTTPRouteMapper` 供 HTTP 协议复用。示例文档亦展示如何在插件中注册 HTTP 路由构建器。参见 `../mcp-forgebird/core/interfaces/mapper.go:1`、`../mcp-forgebird/core/mapper/http_route_mapper.go:1`、`../mcp-forgebird/core/mapper/mapper.go:12`。
...
- 响应体验增强：`ResponseProcessor` 会统一构建 `_meta`（状态码、状态文本、请求方法/URL、首部），支持 204/空响应以及文本降级路径，与 fastmcp 的 `ToolResult` 行为保持一致；`extractOutputSchema` 同步写入 `x-fastmcp-wrap-result` 扩展，并在请求构建阶段自动推导 `Accept` 头。参见 `pkg/openapimcp/executor/processor.go`、`pkg/openapimcp/factory/schema.go`、`pkg/openapimcp/executor/builder.go`、`pkg/openapimcp/executor/error_handler_test.go`。
